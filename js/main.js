...
String.prototype.latinise = function () { return this.replace(/[^A-Za-z0-9\[\] ]/g, (a) => { return Latinise.latin_map[a] || a }) };
String.prototype.latinize = String.prototype.latinise;
String.prototype.isLatin = () => { return this == this.latinise() }
...
$('body').on('click', '.science', function (event) {
    ...
    $(this).addClass("nay");
    $("#answered").prepend(`<p class='explain'>${((data.answer == $(this).data('option')) ? data.right : data.wrong)}</p>`);
    $(".science").eq(data.answer).addClass('yay');

    $(".science").removeClass('science');
    $(".explain").html($(".explain").html().replace(/(?:\r\n|\r|\n)/g, '<br />'))
    $("#answered").fadeIn(500, "swing", () => {
...
function changePage(url, bool) {
    ...
    section.load(url + ' .cd-main-content > *', () => {
...
$('body').on('keyup keypress', '.modsearch', function () {
    var searchStr = $(".modsearch").val().toLowerCase().latinize();
    $(".new-mod-container").show();

    searchStr.split(/[^a-z0-9]/ig).forEach((sterm) => {
        $(".new-mod-container").each(function () {
            var pool = [
                $(this).children(".new-mod-overlay").children("h2").text().toLowerCase().latinize(),
                $(this).children(".new-mod-overlay").children("p.description").text().toLowerCase().latinize(),
                $(this).children(".new-mod-overlay").children("p.seo").text().toLowerCase().latinize()
            ].join(" ")
